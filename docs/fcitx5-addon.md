...
**何が届いているかを見る口を付けてある。** 環境変数  を立てて
fcitx5 を起こすと、打鍵のたびに周辺テキストの文字数・カーソル位置・前後の抜粋・相手の
アプリ名を記録に出す。効かないときに**そもそも届いていないのか、届いているが並べ替えに
使われていないのか**を分けられないと、直す先が決まらない。

```sh
pkill fcitx5; TTYSKK_CONTEXT_LOG=1 fcitx5 -r
```

**打った文章がそのまま記録に出る**ので、既定では黙っている。
...
**`subMode` を実装しただけでは、モードの変化は画面に出ない。** これは問われたときに
答える口でしかなく、fcitx5 は ttyskk の中で かな→カナ と移ったことを知らない。切り替え
の瞬間に出る「ttyskk (あ)」の吹き出しは入力メソッドを**切り替えたとき**にしか出ないので、
ttyskk の中でモードを変えても前のまま、という形で現れる (fcitx5-skk では出ていた)。

変わったときに、こちらから二つに知らせる (`notifyModeChange`)。

| 知らせる先 | 呼ぶもの | 性質 |
|---|---|---|
| 札 (トレイ) | `ic->updateUserInterface(UserInterfaceComponent::StatusArea)` | **残る**。いつでも見に行ける |
| 吹き出し | `instance_->showInputMethodInformation(ic)` | **消える**。切り替えた瞬間だけ目に入る |

両方要る。札だけだと視線を隅まで動かすことになり、吹き出しだけだと少し経つと分からなく
なる。吹き出しは利用者が全体設定で切っていれば出ない (それでよい)。

**アイコンはモードごとに分ける** (`subModeIconImpl`)。札の絵が変われば、いま何モードかを
見るのに視線を動かすだけで済む。`fcitx5/icons/` に置いてあり、`generate.py` が作る。

| モード | アイコン | 見た目 |
|---|---|---|
| (一覧などの既定) | `fcitx-ttyskk` | 端末色の角丸に緑の ▽ |
| かな | `fcitx-ttyskk-hiragana` | あ + 緑の下線 |
| カタカナ | `fcitx-ttyskk-katakana` | ア + 緑の下線 |
| 半角カタカナ | `fcitx-ttyskk-halfkana` | 半 + 緑の下線 |
| 全角英数 | `fcitx-ttyskk-fullwidth` | Ａ + 緑の下線 |
| 直接入力 | `fcitx-ttyskk-latin` | **灰色の A、下線なし** |

**直接入力だけ色を沈めてある。** 全角英数の Ａ とは意味がまるで違う (一方は ttyskk が
変換に関わらない状態) のに、22 ピクセルの札では字幅の差が読み取れないため。意味の違いを
色と下線の有無に移した。

作るときの決まりも二つある。

- **文字はパスに落とす** (`PangoCairo.layout_path`)。SVG に `<text>` を残すと、描く側に
  そのフォントが無ければ別の字形になるか消える
- **どの字も同じ倍率で描く。** 字ごとに枠いっぱいへ伸ばすと、全角Ａと半角 A が同じ形に
  なる。かといって字送り (logical) を基準にすると、CJK フォントは行間を含む分だけ余白が
  大きく、**絵に対して字が小さくなりすぎる**。字形の輪郭 (ink) で測り、**いちばん背の
  高い字がちょうど収まる倍率**を全部に使う。余白が消え、字幅の違いも残る

SVG と PNG (16〜64) の両方を配る。札が SVG を描けない環境があるためで、既存の入力
メソッドもそうしている。
