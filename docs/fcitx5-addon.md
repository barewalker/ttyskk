...
3. **候補窓** — いまは入力中の表示に候補が一つずつ出るだけ。`ttyskk_candidate_*` は
   capi に揃っているので、`CommonCandidateList` に繋ぐ
...
- 学習を書き出す間隔。短いとディスクを叩きすぎ、長いと落ちたときに失われる
- 候補一覧の頁の切り方。fcitx5 の `CommonCandidateList` に任せるか、`inline_until` を
  尊重して SKK 流 (最初の数件は一つずつ送る) にするか
