...
手前の一文字を消すのは `BS` と `C-h` の両方 (`keys.backspace`)。ASCII・全角英数モードでも
同じで、ここに挙げたキーは**どの段でも消す意味**で子アプリへ届く — モードによって消せたり
消せなかったりしないようにしてある。`C-h` を子アプリ自身の操作に使いたければ
`backspace = ["bs"]` と書いて譲れる。

逆に**割り当てから外したキーは、拡張鍵盤プロトコルを使うアプリ (Claude Code など) の下では
ttyskk に届かなくなる**。素の形に戻す対象が割り当て一覧だからで、戻せなかったキーは
割り当ての無いキーとして素通りする — ▽ の途中で押すと、そこで見出し語が確定してしまう。
消すキーを増やすときは `backspace` に足しておくこと。
