...
送り仮名ブロック (`おくr /送/[り/送/]/` の `[...]`) もそのまま引き継ぐ — 送り仮名ごとの
仕分けは、候補と同じだけ学習の成果にあたる (下の「送り仮名ごとの学習」)。
...
| `C-b` / `←` | ▽ の見出し語の中でカーソルを左へ |
| `C-f` / `→` | 同じく右へ |
| `C-a` / `C-e` | ▽ の見出し語の先頭 / 末尾へ |
| `C-d` | ▽ のカーソル位置にある一文字を消す |
...
▼ の途中で `C-g` を押すと ▽ に戻る。そこから `C-b` / `C-f` (または矢印) で見出し語の中を
動かし、`BS` で手前の一文字、`C-d` でカーソルの一文字を消して、`space` で変換し直せる。
打ち間違いに気付いたときに、最初から打ち直さずに済む。

```
Kanji space    → ▼漢字
C-g            → ▽かんじ
C-b C-b        → ▽かんじ   (「ん」の上にカーソル)
C-d            → ▽かじ
space          → ▼家事
```

カーソルのある一文字は反転して出る。重ね描きしている間は端末のカーソルを隠しているので
(そうしないと描き替えのたびにちらつく)、位置はこの一文字でしか示せない。

手前の一文字を消すのは `BS` と `C-h` の両方 (`keys.backspace`)。

**消すものが無ければ、押されたキーのまま子アプリへ渡す。** `C-h` を `0x7f` に揃えたりは
しない — `<C-h>` に別の働きを割り当てているアプリで、それが効かなくなるため (nvim の窓の
移動がそう。`0x08` なら `<C-h>`、`0x7f` なら `<BS>` として読まれる)。文字を打つ段では、
どのみちアプリ側が `C-h` を手前の一文字消しに割り当てているので、そのまま渡せば消える。

**割り当てから外したキーは、拡張鍵盤プロトコルを使うアプリ (Claude Code や nvim) の下では
ttyskk に届かなくなる**。素の形に戻す対象が割り当て一覧だからで、戻せなかったキーは
割り当ての無いキーとして素通りする — ▽ の途中で押すと、そこで見出し語が確定してしまう。
消すキーを増やすときは `backspace` に足しておくこと。`C-h` を ttyskk に一切触らせたく
なければ `backspace = ["bs"]` と書く。

移動が効くのは ▽ の中だけで、それ以外では押したキーがそのまま子アプリへ渡る。送り仮名が
あるときに動かせるのも見出し語の中までで、送り仮名は末尾に残る (`▽うご*く`)。打ちかけの
ローマ字は動く前に始末される — 確定するときと同じく、`n` だけが「ん」になって残り、
ほかの子音は捨てられる。
...
> **試験的な機能。** 既定で有効だが、効き方はまだ日常の使い心地で測っている段階に
> ある。**将来の版で作りが変わるか、取り除かれることがある** — そのときは設定に
> 書いたままでも壊れないようにする (知らない項目は読み飛ばす)。

**日本語は同音異義語が多すぎる。** 「こうせい」は `SKK-JISYO.L` に 20 件を超える候補が
あり、見出し語だけでは決めようがない。最近使った順に並べても、話題が変われば外れる。

```
こうせい  構成 公正 校正 攻勢 後世 後生 更生 更正 厚生 恒星 鋼製 …
こうえん  公園 講演 公演 後援 高遠 好演 口演 光塩 高塩 後縁 …
```

ttyskk は**子アプリの画面の控え**を持っている。他の SKK 実装に無い立場で、子が書いた
文字まで文脈にできる。画面はペインごとなので、**話題の切れ目が自然に入る**。

```toml
[behavior]
context_order = true          # 既定。順序を指で覚えているなら false
context_half_distance = 200   # 重みが半分になる距離 (文字数)
```

実際の `SKK-JISYO.L` での効き方。
...
| `welfare policy and social security` | **厚生** 構成 公正 | |
| 「劇団の公演は来週です」 | | **公演** 公園 講演 |
| 「作家をまねいて話をきく会」 | | **講演** 公園 公演 |

手掛かりは二つで、**どちらも辞書に既にある情報**を使う。余分なファイルは持たない。

1. **候補そのものが画面にあるか。** 文章は同じ語を繰り返す
2. **注釈の共起語が画面にあるか。** `校正;proofread.「新聞の-」` のように、
   `SKK-JISYO.L` は用例と訳語を注釈に持っている。**英語も見る**ので `proofread` で
   当たる。上の表の「作家…」の行は、画面に「講演」の語が無くても当たっている例

カーソルから遠いほど軽く見る。**冪関数を使う** — 指数関数は遠くを殺しすぎるからで、
文書の冒頭に一度だけ書かれた語が手掛かりになることは多い。

```
w(d) = 1 / (1 + d/λ)      λ = context_half_distance (既定 200)
```

**距離は文字数**で数える。行数ではない — 一行の文字数はフォントの大きさや多重化器の
分割で変わるので、行を単位にすると環境ごとに効き方がぶれる。
...
λ を小さくすると近くだけを見る (話題が細かく切り替わる文書向き)、大きくすると画面
全体を平らに見る (一つの話題が続く長文向き)。同じ語が何度も出てくれば足し合わせる
ので、「近い」と「よく出る」の両方が効く。

一文字の候補は、**漢字の連なりがちょうどその一文字**のときだけ数える。「公」が
「公園」「公演」の中に当たっては信号にならない。英語は語の境界で見て、`src/main.rs` や
`--config-example` のような**識別子やパスの一部は数えない** (人の文章ではない)。

**手掛かりが無ければ並びを変えない。** 関わりの無い画面で悪くなることはない。費用は
変換一回あたり 0.1〜0.2 ミリ秒で、打鍵のたびではなく変換のたびにしか掛からない。

引き換えに **同じ読みが画面によって違う順で出る**ようになる。順序を指で覚えている人は
`context_order = false` にする。

**GUI (fcitx5) でも効くが、届く範囲がまるで違う。** 端末では子アプリの画面の控えを
使えるので、**子が書いた文字まで**手掛かりになる。GUI で使えるのは fcitx5 の周辺
テキスト、つまり**いま打ち込んでいる入力欄の中身**だけで、画面に何が映っていようと
入力メソッドからは見えない。

- **効く場** — 編集器・メールの本文・長い textarea。書きかけの文章そのものが手掛かり
- **効かない場** — 検索窓・一行フォーム・アドレス欄。中身が数文字しかないので手掛かりが
  無い。**画面に答えの語が見えていても関係ない** (周辺テキストに入らないため)
- **そもそも届かない場** — 周辺テキストは子アプリが送ってくるもので、送らないアプリも
  ある (GTK/Qt の入力欄は概ね送る。端末エミュレータの中などは持たない)

いずれも手掛かりが無ければ並びを変えないので、悪くなることはない。

効き方を測る例文が `tests/context-sample.md` にある。六つの節がそれぞれ違う話題を持ち、
**答えの漢字は文書のどこにも書いていない**ので、当たれば注釈の共起語が効いたことに
なる。設定を変えて比べるときの基準として使う (`tests/context_sample.rs` で固定してある)。
...
**送りありの見出し語は、送り仮名が違っても同じになる。** 「大きい」は `OoKii`、
「多く」は `OoKu` と打つが、辞書を引く見出し語はどちらも `おおk` (読み + 送り仮名の
最初のローマ字一文字)。候補は語幹だけを持つ。

```
$ grep '^おおk ' SKK-JISYO.L
おおk /大/多/夛;「多」の異体字/
```

見出し語だけで学習すると、片方がもう片方を引きずる。「大きい」を確定したあとに
「おおく」と打つと `▼大く` が出てしまう。そこで**確定するたびに送り仮名ごとの
宛先も覚える**。
...
ddskk・CorvusSKK と同じ書き方なので、利用者辞書を分け合っても互いに読める。**他の
実装が書いた宛先も消さない** — 読み込みで保ち、取り込み (`--import`) でも引き継ぐ。

引くときの効かせ方は選べる。

```toml
[behavior]
okuri_match = "first"   # その送り仮名の候補を先頭へ寄せる (既定)
okuri_match = "only"    # その送り仮名の候補だけを出す
okuri_match = "off"     # 使わない (ddskk の既定と同じ)
```

既定が `first` なのは**候補を隠さない**から。宛先が貯まっていない見出し語では並びが
変わらないので、入れて悪くなることがない。宛先は使ううちに貯まるので、一度直せば
次から正しく出る。候補を削除 (`X`) すると宛先からも消える。
...
**伸ばした見出し語が何になるかを添える。** 見出し語だけでは選べない — 「かんじゃ」が
患者なのか冠者なのかは、変換するまで分からないからで、それでは TAB を送るたびに
一度 `space` で確かめて戻る往復になる。出るのは `space` を押したら出る候補・その注釈
(`; …`)・**いま何番目か**の三つで、一周したのかまだ先があるのかも分かる。
...
住所や署名のように長いものは、登録画面で打つより**定型文として書く**方が楽。
何も打っていないところで `space` を押すと `[スニペットを登録]` が出て、決めると
編集器が開く (次の節)。

### 定型文 (スニペット)

住所・電話番号・メールの署名のように「打つのが面倒で、内容が決まっている」文字列は、
**手元の編集器で一覧を見ながら書く**。見出し語を先に決めなくてよいし、既にあるものを
見ながら足せる。

```sh
ttyskk --edit-snippets            # 一覧を開く
ttyskk --edit-snippets かいしゃ   # 見出し語を埋めた雛形を足して開く
```

新しい項目の雛形を末尾に足し、**その行にカーソルを置いて**開くので、書き足す場所を
探すところから始めなくてよい。手つかずのまま閉じれば雛形は残らない。変換の途中から
なら、候補が尽きて登録に入ったところで `space` を押しても同じ編集器に行ける。

書式は **VS Code のスニペット** (`*.code-snippets`)。TextMate 由来のこの書式は LSP の
仕様にも入っていて、事実上の共通語になっている。nvim の LuaSnip も `from_vscode` で
同じファイルを読めるので、**編集器と入力メソッドで一つのファイルを分け合える**。
注釈 (`//`) と末尾のカンマを書いてよい。
...
`prefix` が見出し語、`body` が候補、`description` が候補の右に出る注釈になる。
同じ見出し語を二つ書けば `space` で選べるので、**日本語版と英語版を並べて注釈で
見分けられる**。`body` を行の並びにすると改行で繋がるので、SKK の辞書には書けない
「改行を含む定型文」も持てる。

**学習では動かさない。** 並びはファイルに書いたとおりに出るし、確定しても利用者辞書へ
写さない。写すと、住所を書き換えたときに古い住所が先頭に残り続ける。編集器で保存した
時点で読み直すので、ttyskk を起動し直さなくてよい。

置き場所は既定で利用者辞書と同じ `~/.local/share/ttyskk/snippets.code-snippets` なので、
辞書を git に載せていれば定型文もそのまま行き来する。学習と分けたい場合や、他の人と
分け合いたい場合は `[snippets]` で移せる。
...
`body` には日付を書いておける。**候補に出る時点でもう開いている**ので、確定してから
気付くのではなく選んでいる最中に確かめられる。
...
`CURRENT_HOUR` `CURRENT_MINUTE` `CURRENT_SECOND` `CURRENT_SECONDS_UNIX` が使える。
月名と曜日名は英語 (`July`、`Tue`)。知らない名前はそのまま残るので `$HOME` のような
文字列は素通りする。

#### 穴を埋める

`$1` を書いておくと、確定せずに**一つずつ埋める段**へ移る。埋め終わってから組み上げて
渡すので、途中の姿は子アプリに見えない (shell でも編集器でも同じように動く)。
...
| `${1\|日本語,英語\|}` | 選ぶ場所。`space` で回す |
| `$0` | 埋め終わったあとのカーソル位置 |
| `\$` | `$` そのもの |

| キー | 動作 |
|---|---|
| `TAB` | 次の場所へ。最後で押すと組み上げて渡す |
| `Shift+TAB` | 前の場所へ |
| `Enter` | 組み上げて渡す (変換の途中ならその変換を決める) |
| `C-g` | やめる。何も出さずに消える |

**埋めながら変換できる。** 打った文字はいまの場所へ溜まり、そこで変換がそのまま使える。
打ちかけのものは埋める場所へ差し込んで見せるので、全体のどこに入るのかがその場で分かる。

```
[埋め 1/2]▼田中 様、⏎⏎いつもお世話になっております。名前 です。
```

同じ番号を二度書けば、どちらにも同じ値が入る。`$0` の戻しは左矢印を送る形なので、
**行をまたぐ場合は戻さない** (上の行へ移る動きは編集器ごとに違って当てにできない)。

キーを一つ割り当てれば、変換に入る前からでも編集器を開ける。既定は空 —
ローマ字にならない英字は `l` と `q` しかなく (SKK がその二つを ASCII とかなカナに
充てているのは空いていたからで)、制御キーの空きは環境ごとに違うため、決め打ちしない。
...
move_left = "left"              # ▽ の中の移動を矢印だけにする (C-b を子アプリへ譲る)
...
`left` `right` `up` `down` `home` `end`、`q` `/` のような一文字そのもの。場所は
`XDG_CONFIG_HOME` を尊重し、`TTYSKK_CONFIG` で直接指定もできる。

矢印と `home` / `end` は端末によって送られる形が違う (`CSI D` / `SS3 D` / `CSI 1~` など)。
名前で書けばその全部が割り当てになるので、どの端末でも同じように効く。
...
`ascii_keys` は押されたキーを見張るので、**それ以外の抜け方には付いていけない**。
`inoremap <C-l> <Esc>` のような割り当てや、プラグインが挿入モードを閉じる場合が該当する。
挿入モードは抜けたのに ttyskk はかなモードのまま残り、次に打った英字がかなになる。

カーソルの形を見れば抜け方に依らない。vim / nvim は挿入モードで棒、ノーマルモードで
ブロックにするので、棒でなくなったら降ろす。
...
**打ちかけがあるうちは降ろさない** (変換・辞書登録・定型文を埋めている途中)。
打ち込んだものが行き場を失わないようにしてある。
...
カタカナ) `Shift+Tab` (前の補完候補)、それに ▽ の見出し語を直す `Ctrl+B` `Ctrl+F`
`Ctrl+A` `Ctrl+E` `Ctrl+D`。割り当てを変えれば解釈する対象も一緒に変わるので、キーを
付け替えても拡張鍵盤プロトコルの下だけ効かない、ということは起きない。

見出し語を直すキーが働くのは ▽ の中だけだが、素の形に戻す対象には常に入る。`CSI 98;5u`
(拡張の形) と `0x02` (素の形) を区別する子アプリでは、`Ctrl+B` が後者で届くようになる。
行編集に使うアプリはどちらも同じ意味に取るので実害は出にくいものの、譲りたければ
`move_left = "left"` のように矢印だけを割り当てればよい。
...
`TTYSKK_JISYO` (`:` 区切り)、`TTYSKK_USER_JISYO` で変えられる。定型文は
`[snippets]` で移す。EUC-JP と UTF-8 のどちらでも読める。

引く順は**利用者辞書 → 定型文 → 共有辞書**。定型文が共有辞書より前に来るのは手で
書いたものだからで、「でんわ」に自分の番号を書いたなら `SKK-JISYO.L` の「電話」より
先に出したい。
...
このエンジンを GUI の入力メソッドに載せた作りは
[`docs/fcitx5-addon.md`](docs/fcitx5-addon.md) にまとめてある。

## まだ無いもの

- 動的補完 (打つそばから候補を出す) — `TAB` の補完はある

## 入れないもの

- **候補一覧の縦並び** — ddskk の既定はエコーエリアへの横並びで、CorvusSKK も縦は
  設定項目。つまり既定は横。縦なのは skkeleton だけで、それは Vim の popup menu に
  乗せているため。行内の横並びと、カーソルの下に一行で浮かせる形 (`float`) で足りる
- **数値変換の `#4` (数値再変換)** — 数字を見出し語として辞書を引き直す指定。
  `SKK-JISYO.L` にあるのは見出し語 `#` の候補の一つだけで、
  **`SKK-JISYO.noregist` (登録すべきでない項目の一覧) に 9 件**あり、いずれも
  「再帰数値変換の意味がない?」と注記されている。標準辞書の側が使わないよう促して
  いる指定なので追わない。いまはそのままの数字を出す
- **skkserver** — 辞書ファイルを共有すれば足りる
- **確定した文字列の再変換** — 子アプリの持ち物になった文字は ttyskk からは触れない。
  直したい語は確定する前に (▽ の中で) 直す — 上の「見出し語を直す」を使う

▽ の外では、矢印キーは見出し語を確定してから子へ渡す。SKK は語単位でその場で確定させる
ので、文節変換型の入力メソッドのような「文節をまたぐカーソル移動」は要らない。
