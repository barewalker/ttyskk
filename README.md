...
ttyskk は**設定でキーに割り当てられている Ctrl 付きのキー**に限ってこの形を解釈
する。既定なら `Ctrl+J` (かな) `Ctrl+G` (取り消し) `Ctrl+Q` (半角カタカナ)
`Ctrl+C` (ASCII へ戻す) の四つ。割り当てを `[keys]` で変えれば解釈する対象も一緒に
...
裏を返すと、**子アプリが同じキーに割り当てている操作は使えなくなる**。Claude Code
で改行を入れたいときは `Shift+Enter` か `\` + `Enter` を使う。
