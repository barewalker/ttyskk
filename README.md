...
**`cell` は文字を足さない。** カーソルのあるセルの色だけを変えるので、行の長さも
中身も変わらない。カーソルそのものに色が付いたように読める。この方式のときは
カーソルの形を下線に固定する — ブロックだと色を覆ってしまうため。つまり
**形 = ttyskk が動いている合図、色 = モード**という配分になる。

| モード | `cell` の色 | `letter` の印 | `off` のときの形 |
|---|---|---|---|
| ASCII | **色を敷かない** | **印なし** | 下線 |
| ひらがな | 緑 | あ | ブロック |
| カタカナ | 水色 | ア | バー |
...
**端末多重化器を挟むとカーソルの色 (OSC 12) は途中で吸われる** (herdr が実際にそう)。
一方、文字として書いた色はそのまま届く。`cell` と `letter` はこの性質を使っている。
`off` は形だけで五つのモードを区別するので、色がまったく届かない環境でも困らない。
