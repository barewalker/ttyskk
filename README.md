...
モードは**カーソルのそばの印**と**カーソルの形**で分かる。何も打っていない状態でも
見て判断できる。
...
印はカーソルの直後に反転表示で出て、カーソルと一緒に動く。**端末多重化器を挟むと
カーソルの色 (OSC 12) は途中で吸われる** (herdr が実際にそう) 一方、文字として書いた
色はそのまま届く。色でモードを見分けたいならこちらを使う。

**ASCII モードでは印を描かない**ので、そのときの完全透過は保たれる。`[behavior]
mode_marker = false` で印を止められる。

カーソルの形は起動している間ずっと設定するので、**形が普段と違うこと自体が
「ttyskk が動いている」合図**になる。包んでいなければ端末の既定のままになる。
形だけで五つのモードを区別できるようにしてあるので、色が届かない環境でも困らない。
...
mode_marker = false             # カーソルのそばのモードの印を出さない
