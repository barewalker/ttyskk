...
手前の一文字を消すのは `BS` と `C-h` の両方。`C-h` を子アプリ自身の操作に使いたければ
`backspace = ["bs"]` と書いて譲れる。逆に**割り当てから外したキーは、拡張鍵盤プロトコルを
使うアプリ (Claude Code など) の下では ttyskk に届かなくなる**ので、▽ の途中で押すと
見出し語がそこで確定してしまう。消すキーを増やすときも `backspace` に足しておくこと。
